...
#[component]
pub fn BuyHouse(current_operate_signal: Signal<CurrentOperate>) -> Element {
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            spawn(async move {
                                ...
                                match buy_house(&house_position).await {
...
async fn buy_house(house_position: &str) -> Result<Signature> {
    let house_position = house_position.parse::<u8>()?;
    let ix = program::instruction::BuyHouse { house_position };
