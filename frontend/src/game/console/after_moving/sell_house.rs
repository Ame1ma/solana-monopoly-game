...
#[component]
pub fn SellHouse(current_operate_signal: Signal<CurrentOperate>) -> Element {
            ...
            div { class: "grid grid-cols-2 w-full gap-y-7 gap-x-9",
                ...
                div { class: "w-full flex flex-col items-center justify-center",
                    button {
                        ...
                        onclick: move |_| {
                            spawn(async move {
                                let house_position = inputed_position();
                                match sell_house(&house_position).await {
...
async fn sell_house(house_position: &str) -> Result<Signature> {
    let house_position = house_position.parse::<u8>()?;
    let ix = program::instruction::SellHouse { house_position };
