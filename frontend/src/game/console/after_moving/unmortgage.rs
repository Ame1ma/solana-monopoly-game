...
#[component]
pub fn Unmortgage(current_operate_signal: Signal<CurrentOperate>) -> Element {
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            spawn(async move {
                                ...
                                match unmortgage(&square_position).await {
...
async fn unmortgage(square_position: &str) -> Result<Signature> {
    let square_position = square_position.parse::<u8>()?;
    let ix = program::instruction::Unmortgage { square_position };
